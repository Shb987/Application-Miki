...
def _save_fallback_pdf(json_paper: dict, filename: str):
    ...
    if fitz and (hasattr(fitz, "open") or hasattr(fitz, "Document")):
        try:
            ...
            y = 45
            page.insert_text((190, y), "MY QUESTION PAPER", fontsize=16, fontname="helv-bold")
            y += 20
            ...
            if isinstance(sections, list):
                for sec in sections:
                    if not isinstance(sec, dict): continue
                    sname = sec.get("section") or sec.get("title") or "Part"
                    if y > 720:
                        page = doc.new_page(width=595, height=842)
                        y = 50
                    page.insert_text((50, y), f"PART {sname}", fontsize=12, fontname="helv-bold")
                    y += 18
            ...
            doc.close()
            pdf_done = True
            print(f"[PyMuPDF-PDF] Saved fallback canvas primary PDF to {filename}")
...
def _save_reportlab_fallback(json_paper: dict, filename: str):
    ...
    try:
        ...
        c.drawCentredString(width / 2.0, y, "MY QUESTION PAPER")
        y -= 20
        ...
        if isinstance(sections, list):
            for sec in sections:
                if not isinstance(sec, dict): continue
                sname = str(sec.get("section") or sec.get("title") or "Part")
                ...
                c.drawString(50, y, f"PART {sname}")
                y -= 20
        ...
        c.save()
        print(f"[ReportLab-PDF] Saved fallback primary PDF to {filename}")
    except Exception as ex:
